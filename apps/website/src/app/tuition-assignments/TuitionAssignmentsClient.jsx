...
export default function TuitionAssignmentsClient({ initialAssignments }) {
  ...
  // --- Data Fetching is removed from here ---
  // The useEffect for fetching assignments is no longer needed in the client.
  ...
  // --- Render Logic ---
  // The loading and error states can be simplified or handled within page.jsx if needed,
  // but for now, we'll keep them in case of client-side errors.
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "description": `Find ${filteredAssignments.length}+ verified ${subjectFilter || ''} tuition assignments ${levelFilter ? `for ${levelFilter}` : ''} in Singapore. Apply instantly for home tutoring jobs.`,
            "hiringOrganization": {
              "@type": "Organization",
            ...
            "datePosted": allAssignments.length > 0 ? new Date(allAssignments[0].createdAt).toISOString() : new Date().toISOString(),
            "validThrough": new Date(new Date().setDate(new Date().getDate() + 30)).toISOString(), // Valid for 30 days
            "baseSalary": {
              "@type": "MonetaryAmount",
              "currency": "SGD",
              "value": {
